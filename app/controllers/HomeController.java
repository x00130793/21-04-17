...
public class HomeController extends Controller {
	...
	for(int i = 0; i < sList.size(); i++){
		temp.add(sList.get(i).getSizeName());
	}
    Size s1 = sizeForm.get();

    Size pp = Size.findByName(s1.getSizeName());
    


        return ok(productDetails.render(categoriesList, getUserFromSession(), cat, sId, filter, id, p, c, sList, commentForm, sizeForm, size, temp));
    }
